...
function parseGetPlaylist(html) {
    ...
    if (videos.length == 0) {
        ...
        playlist = __assign(__assign({ id: primaryRenderer.title.runs[0].navigationEndpoint.watchEndpoint.playlistId, title: primaryRenderer.title.runs[0].text, videoCount: +primaryRenderer.stats[primaryRenderer.stats.length - 3].runs[0].text.replace(/[^0-9]/g, ""), viewCount: +primaryRenderer.stats[primaryRenderer.stats.length - 2].simpleText.replace(/[^0-9]/g, ""), lastUpdatedAt: primaryRenderer.stats[primaryRenderer.stats.length - 1].simpleText }, videoOwner !== undefined && {
            channel: {
...
function parseGetVideo(html) {
    try {
        ...
        var videoInfo = __assign(__assign({}, secondaryInfo), primaryInfo);
        var tags_2 = [];
        var description_2 = "";
